...
export function MarginCalculator({ idPrefix, hero = false }: { idPrefix: string; hero?: boolean }) {
  const defaults = { sales: '2500000', fixed: '900000', variable: '12' };
  const [sales, setSales] = useState(defaults.sales); const [fixedCosts, setFixedCosts] = useState(defaults.fixed);
  const [variableRate, setVariableRate] = useState(defaults.variable);
  const salesValue = parseNumber(sales); const fixedValue = parseNumber(fixedCosts); const variableValue = parseNumber(variableRate);
  ...
  return (
    <div className={`tool-form ${hero ? 'hero-tool-form' : ''}`}>
      ...
      {isValid ? <div className="result-panel tool-results" aria-live="polite" aria-atomic="true">
...
function BreakEvenCalculator() {
  const [fixedCosts, setFixedCosts] = useState('900000'); const [price, setPrice] = useState('25000');
  ...
  const isValid = !fixedError && !priceError && !variableError; const contribution = isValid ? salePrice! - variable! : 0;
  const units = isValid ? Math.ceil(fixed! / contribution) : 0; const revenue = isValid ? units * salePrice! : 0;
  return <div className="tool-form">
    ...
    {isValid ? <div className="result-panel tool-results three-results" aria-live="polite" aria-atomic="true">
      <ResultItem label="Contribución por unidad" value={money.format(contribution)} /><ResultItem label="Unidades para equilibrar" value={integer.format(units)} />
...
function CommissionCalculator() {
  ...
  const gross = isValid ? average! * count! : 0; const costA = isValid ? gross * (aRate! / 100) + count! * aFixed! : 0;
  const costB = isValid ? gross * (bRate! / 100) + count! * bFixed! : 0; const netA = gross - costA; const netB = gross - costB;
  const difference = Math.abs(netA - netB); const winner = Math.abs(netA - netB) < 0.01 ? 'Ambos canales dejan el mismo ingreso neto.'
  ...
  return <div className="tool-form">
    ...
    <div className="channel-grid">
      <fieldset className="channel-card"><legend>Canal A</legend><NumberField id="commission-rate-a" label="Comisión porcentual" suffix="%" value={rateA} onChange={setRateA} max={100} step={0.01} error={rateErrorA} /><NumberField id="commission-fixed-a" label="Tarifa fija por transacción" prefix="$" value={fixedA} onChange={setFixedA} step={10} error={fixedErrorA} /></fieldset>
      <fieldset className="channel-card"><legend>Canal B</legend><NumberField id="commission-rate-b" label="Comisión porcentual" suffix="%" value={rateB} onChange={setRateB} max={100} step={0.01} error={rateErrorB} /><NumberField id="commission-fixed-b" label="Tarifa fija por transacción" prefix="$" value={fixedB} onChange={setFixedB} step={10} error={fixedErrorB} /></fieldset>
    </div>
    {isValid ? <div className="result-panel comparison-results" aria-live="polite" aria-atomic="true">
      <ResultItem label="Ventas brutas" value={money.format(gross)} /><ResultItem label="Costo Canal A" value={money.format(costA)} /><ResultItem label="Ingreso neto A" value={money.format(netA)} />
      <ResultItem label="Costo Canal B" value={money.format(costB)} /><ResultItem label="Ingreso neto B" value={money.format(netB)} /><ResultItem label="Diferencia" value={money.format(difference)} />
...
function UtmBuilder() {
  ...
  try {
    ...
    if (source.trim() && medium.trim() && campaign.trim()) {
      const params = new URLSearchParams(url.search); params.set('utm_source', source.trim()); params.set('utm_medium', medium.trim());
      params.set('utm_campaign', campaign.trim()); if (term.trim()) params.set('utm_term', term.trim()); else params.delete('utm_term');
      if (content.trim()) params.set('utm_content', content.trim()); else params.delete('utm_content'); url.search = params.toString(); generatedUrl = url.toString();
    }
  } catch { urlError = 'Ingresa una URL completa que comience con http:// o https://.'; }
  const sourceError = source.trim() ? undefined : 'utm_source es obligatorio.'; const mediumError = medium.trim() ? undefined : 'utm_medium es obligatorio.';
  ...
  async function copyLink() { if (!isValid) return; try { await navigator.clipboard.writeText(generatedUrl); setCopyStatus('Enlace copiado al portapapeles.'); } catch { setCopyStatus('No pudimos copiar automáticamente. Selecciona el enlace y cópialo manualmente.'); } }
  return <div className="tool-form">
    ...
    {isValid ? <div className="utm-result" aria-live="polite"><span>Enlace generado</span><output>{generatedUrl}</output><button className="button copy-button" type="button" onClick={copyLink}>Copiar enlace</button><p className="copy-status" role="status">{copyStatus}</p></div>
      : <InvalidResult message="Completa la URL y los tres parámetros obligatorios para generar el enlace." />}
...
function EstimateCalculator() {
  ...
  const [complexity, setComplexity] = useState<Complexity>('medium'); const [safety, setSafety] = useState('15'); const [externalCosts, setExternalCosts] = useState('100000');
  ...
  const isValid = !hourError && !rateError && !safetyError && !externalError; const multipliers: Record<Complexity, number> = { low: 1, medium: 1.2, high: 1.4 };
  const multiplier = multipliers[complexity]; const baseCost = isValid ? hourValue! * rateValue! : 0; const complexityAdjustment = baseCost * (multiplier - 1);
  ...
  return <div className="tool-form">
    <div className="tool-field-grid three-columns">
      ...
      <label className="field-group" htmlFor="estimate-complexity"><span className="field-label">Complejidad</span><span className="input-shell select-shell"><select id="estimate-complexity" value={complexity} onChange={(event) => setComplexity(event.target.value as Complexity)}><option value="low">Baja · ×1,00</option><option value="medium">Media · ×1,20</option><option value="high">Alta · ×1,40</option></select></span></label>
    ...
    {isValid ? <div className="result-panel tool-results quote-results" aria-live="polite" aria-atomic="true"><ResultItem label="Costo base" value={money.format(baseCost)} /><ResultItem label="Ajuste por complejidad" value={money.format(complexityAdjustment)} /><ResultItem label="Contingencia" value={money.format(contingency)} /><ResultItem label="Costos externos" value={money.format(externalValue!)} /><ResultItem label="Cotización recomendada" value={money.format(quote)} tone="positive" /></div> : <InvalidResult />}
  </div>;
}

function PricingCalculator() {
  const [currentPrice, setCurrentPrice] = useState('35000'); const [unitCost, setUnitCost] = useState('12000');
  const [fixedPerSale, setFixedPerSale] = useState('5000'); const [commission, setCommission] = useState('3.2'); const [targetMargin, setTargetMargin] = useState('30');
  const price = parseNumber(currentPrice); const unit = parseNumber(unitCost); const fixed = parseNumber(fixedPerSale); const commissionRate = parseNumber(commission); const target = parseNumber(targetMargin);
  ...
  const currentProfit = isValid ? price! - unit! - fixed! - price! * (commissionRate! / 100) : 0; const currentMargin = isValid ? (currentProfit / price!) * 100 : 0;
  const denominator = isValid ? 1 - commissionRate! / 100 - target! / 100 : 0; const recommendedPrice = isValid ? (unit! + fixed!) / denominator : 0;
  const difference = recommendedPrice - (price ?? 0); const diagnostic = currentMargin >= (target ?? 0) ? { label: 'Precio saludable', detail: 'El margen actual alcanza o supera tu objetivo.', tone: 'positive' }
  ...
  return <div className="tool-form">
    ...
    {isValid ? <div className="result-panel pricing-results" aria-live="polite" aria-atomic="true"><ResultItem label="Utilidad actual por venta" value={money.format(currentProfit)} tone={currentProfit < 0 ? 'negative' : 'positive'} /><ResultItem label="Margen actual" value={`${currentMargin.toFixed(1)}%`} tone={currentMargin < 0 ? 'negative' : 'positive'} /><ResultItem label="Precio mínimo recomendado" value={money.format(recommendedPrice)} /><ResultItem label="Diferencia contra precio actual" value={`${difference > 0 ? '+' : ''}${money.format(difference)}`} tone={difference > 0 ? 'negative' : 'positive'} /><div className={`diagnostic-card ${diagnostic.tone}`}><strong>{diagnostic.label}</strong><span>{diagnostic.detail}</span></div></div>
      : <InvalidResult message="Ajusta los porcentajes para que comisión y margen objetivo sumen menos de 100%." />}
...
export function ToolCalculator({ toolId, idPrefix = 'tool-page' }: { toolId: ToolId; idPrefix?: string }) {
  switch (toolId) {
    case 'margin': return <MarginCalculator idPrefix={`${idPrefix}-margin`} />;
