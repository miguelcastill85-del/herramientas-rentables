...
const BRAND = 'SIGMANOUVA';
const CANONICAL_LOGO_SHA256 = 'f270af923e3075c1dcf53e9e6b27291e32d772fe5e080248d2fd742c2691768b';
const CANONICAL_LOGO_NATIVE = { width: 1024, height: 1024 };
const LOGO_CROPS = {
  symbol: { x: 245, y: 185, width: 540, height: 500 },
  lockup: { x: 65, y: 145, width: 895, height: 675 },
};
const PALETTE = {
  ink: '#111214',
  paper: '#FEFEFC',
...
function fail(message) {
  console.error(`SIGMANOUVA_RENDER_ERROR: ${message}`);
  process.exit(1);
}
...
function validateInput(d) {
  if (d.brand !== BRAND) fail(`brand must be ${BRAND}`);
  ...
  if (d.dark) fail('dark mode is disabled until a separately approved transparent derivative exists');
}

function svgOpen(width, height, d) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
<metadata>brand=${BRAND};publicationId=${esc(d.publicationId)};knowledgeId=${esc(d.knowledgeId)};logoSha256=${CANONICAL_LOGO_SHA256}</metadata>
...
<text class="muted" x="72" y="${height - 58}" font-size="24">SIGMANOUVA — Lo que importa, explicado.</text>`;
}
...
<text class="label" x="76" y="220" font-size="24">SIGMANOUVA · ${esc((d.franchise ?? 'ANÁLISIS').toUpperCase())}</text>
...
function renderT02(d, logo, outputPath) {
  ...
  console.log(`SIGMANOUVA_RENDER_PASS ${d.publicationId} ${d.slides.length} slides -> ${dir}`);
}

const [inputPath, outputPath] = process.argv.slice(2);
if (!inputPath || !outputPath) fail('usage: node render-sigmanouva-svg.mjs <input.json> <output.svg|output-base.svg>');
...
if (d.template === 'T02') {
  ...
  console.log(`SIGMANOUVA_RENDER_PASS ${d.publicationId} -> ${outputPath}`);
}
