...
function officialUrl(code){return code?'https://buscador.mercadopublico.cl/ficha?code='+encodeURIComponent(code):null}
function renderProducts(item){
  const products=Array.isArray(item.productos)?item.productos.slice(0,4):[];
  if(!products.length)return '';
  return '<ul class="products">'+products.map(p=>{
    const qty=p.cantidad!=null?' · '+safe(p.cantidad)+(p.unidad?' '+safe(p.unidad):''):'';
    return '<li><strong>'+safe(p.nombre||'Producto')+'</strong>'+qty+'</li>';
  }).join('')+'</ul>';
}
function renderOpportunity(item){
  ...
  const offers=item.ofertas_recibidas==null?'Ofertas no informadas':item.ofertas_recibidas+' ofertas recibidas';
  const official=officialUrl(item.codigo);
  const action=official?'<div class="actions"><a class="official-link" href="'+safe(official)+'" target="_blank" rel="noopener noreferrer">Ver ficha oficial ↗</a></div>':'';
  return '<article class="opportunity"><div class="opportunity-head"><div><span class="code">'+safe(item.codigo||'SIN CÓDIGO')+'</span><h3>'+safe(item.nombre||'Compra Ágil')+'</h3></div><span class="pill '+(item.ticket_band==='secundario'?'secondary':'')+'">'+safe(bandLabel(item.ticket_band))+'</span></div><div class="meta"><span>'+safe(budget)+'</span><span>•</span><span>'+safe(offers)+'</span><span>•</span><span>'+safe(item.organismo||'Organismo no informado')+'</span><span>•</span><span>'+safe(item.region||'Región no informada')+'</span><span>•</span><span>Cierre '+safe(close)+'</span><span>•</span><span>Score '+safe(item.score??'—')+'</span></div>'+renderProducts(item)+action+'</article>';
}
async function load(){
  ...
  try{
    ...
    if(live.mode==='api_v2'){
      const items=Array.isArray(live.items)?live.items:[];el('total').textContent=String(items.length);el('op-note').textContent='Procesos publicados que coinciden con el foco del Radar. Abre la ficha oficial antes de decidir.';
